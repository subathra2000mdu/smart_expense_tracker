        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-5 bg-white p-6 rounded-2xl border shadow-sm">
            ...
            <form onSubmit={handleAdd} className="space-y-4">
              ...
              <button className={`w-full py-3 rounded-xl cursor-pointer font-bold text-white ${editingId ? 'bg-amber-500' : 'bg-indigo-600'}`}>
        ...
        <div className="space-y-4">
          <div className="bg-white p-4 rounded-2xl border flex flex-col sm:flex-row justify-between items-center gap-4">
            <div className="flex items-center gap-4 w-full sm:w-auto">
              <select className="font-bold p-2 bg-slate-50 rounded-lg flex-1 cursor-pointer" value={filter} onChange={e => setFilter(e.target.value)}>
                <option value="All">All Categories</option>
                {CATEGORIES.map(c => <option key={c} value={c}>{c}</option>)}
              </select>
              <button onClick={clearAll} className="text-red-500 text-sm font-bold flex items-center gap-1 cursor-pointer">
            ...
            {processedExpenses.map(exp => (
              <div key={exp.id} className="bg-white p-5 rounded-2xl border flex items-center justify-between">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={() => startEdit(exp)} className="text-slate-400 hover:text-amber-500 cursor-pointer"><Edit3 size={18} /></button>
                  <button onClick={() => handleDelete(exp.id)} className="text-slate-400 hover:text-red-500 cursor-pointer"><Trash2 size={18} /></button>
                </div>
              </div>
            ))}
          </div>
